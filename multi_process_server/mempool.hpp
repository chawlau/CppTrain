    ...
    class Process_pool
    {
            ...
            Process_pool(const std::string& port):server(new SOCKET::Socket(port))
        {
            ...
            void child_detect()
            {
                ...
                {
                    //std::cout<<"wait client num"<<wait_client.size()<<std::endl;
                    //std::cout<<child_info[index].ch_pid<<": busy"<<child_info[index].ch_busy<<std::endl;
                    if(child_info[index].ch_busy==0)
                    {
                        if(!wait_client.empty())
                        {
                            //std::cout<<"handle queue's client "<<std::endl;
